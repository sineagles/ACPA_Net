...
from unet.networks_other import init_weights
...
class _GridAttentionBlockND(nn.Module):
    ...
    def _concatenation(self, x, g):
        ...
        sigm_psi_f = torch.sigmoid(self.psi(f))     #得到的这个是权重图
...
if __name__ == '__main__':
    ...
    for mode in mode_list:
        ...
        # print(out.size())
        img= Variable(torch.rand(4, 64,512,512))
        gat = Variable(torch.rand(4, 128, 256, 256))
        net = GridAttentionBlock2D(in_channels=64,inter_channels=8,gating_channels=128,mode=mode,sub_sample_factor=(2,2))
        out, sigma = net(img, gat)
        print(out.size())
